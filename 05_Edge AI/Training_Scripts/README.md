...
│       ├── dataset/               # Blender-Bilder + Masken + Labels + labels_pose/
│       ├── yolo_dataset/          # YOLO-Seg Ordnerstruktur (Train/Val Split)
│       ├── pose_dataset/          # YOLO-Pose Ordnerstruktur (Train/Val Split)
...
|---|---|---|
| [1] | `train_virtuell()` | **Seg + Pose** auf Blender-Daten (je 40 Ep.) |
| [2] | `train_echt()` | **Seg Fine-Tuning + Pose** auf echten Bildern (Seg: 5 Ep., Pose: 50 Ep.) |
...
1. **`blender_masken_zu_labels()`** — Farbige Blender-Masken (`mask_*.png`) → YOLO-Seg-Labels  
...
2. **`csv_zu_pose_labels()`** — Kopiert vorgenerierte Pose-Labels aus `labels_pose/` → Train/Val Split  
   6 Keypoints (Base + Joint 1-4 + TCP/Greiferspitze), Format: `class cx cy w h kp0_x kp0_y kp0_v ... kp5_x kp5_y kp5_v`
...
2. **`seg_zu_pose_labels_real()`** — Seg-Schwerpunkte → YOLO-Pose-Labels für echte Bilder  
   Nutzt das trainierte Seg-Modell als "Lehrer" zur Keypoint-Erzeugung. Der TCP-Keypoint wird dabei als nicht sichtbar (`0 0 0`) gesetzt, da er aus den 5 Segmentklassen nicht direkt ableitbar ist.
...
3. **Richtungsvektoren** entlang der Keypoint-Kette `Base → Joint_1 → Joint_2 → Joint_3 → Joint_4 → TCP`
4. **Joint 1:** Winkel des ersten Vektors zur Vertikalen (Referenz: oben = 0°)
...
### 4. `ResearchEdgeAI.ipynb` — MobileNetV3 Regression (Alternative)

Alternativer Ansatz: Direkte Regression der Gelenkwinkel statt Segmentierung.

- **Backbone:** MobileNetV3Large (ImageNet, frozen)
- **3 Heads:** JointNet (7 Winkel), RotationNet (6 Rotationsparams), KeypointNet (21 Keypoints)
- **Loss:** MSE, Gewichtung 1:1:10

*Aktuell nicht im aktiven Workflow — dient als Forschungsalternative.*

---

### 5. `label_studio_converter.py` — Import fuer Label Studio vorbereiten
...
- `tasks_seg_all.json` (500 Tasks)
- `tasks_pose_train.json` (350 Tasks)
- `tasks_pose_val.json` (149 Tasks)
...
### 6. `npzConverter.py` — Kalibrierungsdaten fuer STM32Cube.AI Studio
