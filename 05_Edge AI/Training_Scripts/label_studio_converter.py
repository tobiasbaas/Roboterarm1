...
def _default_root_dir() -> Path:
    # Training_Scripts directory (where this file lives)
...
def _interactive_menu() -> int:
    ...
    print("=" * 60)
    print("[1] Nur Segmente umwandeln")
    print("[2] Nur Pose umwandeln (train + val)")
    print("[3] Segmente + Pose umwandeln (beides)")
    print()
    ...
    root_raw = input(f"Root-Pfad [Enter={default_root}]: ").strip()
    root = Path(root_raw) if root_raw else default_root

    image_host = input("Image-Host [Enter=http://127.0.0.1:3000]: ").strip() or "http://127.0.0.1:3000"
    import_as = input("Import als predictions/annotations [Enter=predictions]: ").strip().lower() or "predictions"
    if import_as not in {"predictions", "annotations"}:
        print("Ungültiger Import-Modus, nutze predictions.")
        import_as = "predictions"

    out_dir = root / "label_studio_import"
    out_dir.mkdir(parents=True, exist_ok=True)

    if choice == "1":
        seg_format = input("Seg-Format polygon/brush [Enter=polygon]: ").strip().lower() or "polygon"
        if seg_format not in {"polygon", "brush"}:
            print("Ungültiges Seg-Format, nutze polygon.")
            seg_format = "polygon"

        seg_output_name = "tasks_seg_all_brush.json" if seg_format == "brush" else "tasks_seg_all.json"
        seg_args = argparse.Namespace(
            images_dir=root / "real_dataset" / "images" / "all",
            labels_dir=root / "real_dataset" / "labels" / "all",
            image_url_prefix=f"{image_host.rstrip('/')}/real_dataset/images/all",
            output=out_dir / seg_output_name,
            import_as=import_as,
            from_name=("brush" if seg_format == "brush" else "label"),
            to_name="image",
            include_empty=False,
        ...
        return _command_seg(seg_args)

    if choice == "2":
        pose_train_args = argparse.Namespace(
            images_dir=root / "real_pose_dataset" / "images" / "train",
            labels_dir=root / "real_pose_dataset" / "labels" / "train",
            image_url_prefix=f"{image_host.rstrip('/')}/real_pose_dataset/images/train",
            output=out_dir / "tasks_pose_train.json",
            import_as=import_as,
            from_name="kp",
            to_name="image",
        ...
        pose_val_args = argparse.Namespace(
            images_dir=root / "real_pose_dataset" / "images" / "val",
            labels_dir=root / "real_pose_dataset" / "labels" / "val",
            image_url_prefix=f"{image_host.rstrip('/')}/real_pose_dataset/images/val",
            output=out_dir / "tasks_pose_val.json",
            import_as=import_as,
            from_name="kp",
            to_name="image",
        ...
        _command_pose(pose_train_args)
        return _command_pose(pose_val_args)
    ...
    all_args = argparse.Namespace(
        ...
        upload=False,
        seg_project_id=None,
        pose_project_id=None,
        label_studio_url="http://localhost:8080",
        api_key="",
        auth_scheme="auto",
    )
    return _command_prepare_all(all_args)
