...
int main(void)
{
  ...
  // --- Enable MPU and CPU Cache before anything else ---
  MPU_Config();
  CPU_CACHE_Enable();
 ...
 // USBPD_PreInitOs();
  ...
  /* USER CODE BEGIN 2 */
  /* Initialize external PSRAM for framebuffers */
  BSP_XSPI_RAM_Init(0);
  BSP_XSPI_RAM_EnableMemoryMappedMode(0);

  /* Initialize LCD with BSP (800x480 RK050HR18 panel) */
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  /* PLL2 = HSI 64MHz / 8 * 125 = 1000 MHz  (DCMIPP kernel clock source) */
  ...
  RCC_OscInitStruct.PLL3.PLLState = RCC_PLL_NONE;
  /* PLL4 = HSI 64MHz / 8 * 225 / (6*6) = 50 MHz  (LTDC pixel clock via IC16/2 = 25MHz) */
...
/* LTDC now initialized by BSP_LCD_Init() called from main */
...
/* I2C1 and I2C2 now initialized by BSP_I2C1_Init() / BSP_I2C2_Init() called from main.
   The BSP bus driver provides __weak MX_I2C1_Init / MX_I2C2_Init with BSP-compatible signatures. */
...
static void MX_GPIO_Init(void)
{
  ...
  // LTDC GPIOs (STM32N6570-DK Beispiel, AF14)
...
/* USER CODE BEGIN 4 */
// --- MPU and Cache Setup for STM32N6xx (Cortex-M55) ---
static void MPU_Config(void)
{
