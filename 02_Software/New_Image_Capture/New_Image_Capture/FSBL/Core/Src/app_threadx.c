...
#define CDC_TX_BUF_SIZE       512   /* match UX_SLAVE_REQUEST_DATA_MAX_LENGTH */
...
static void cmd_thread_entry(ULONG thread_input)
{
  ...
  while (1)
  {
    ...
    if (actual_length >= 7 && memcmp(rx_buf, "CAPTURE", 7) == 0)
    {
      ...
      cdc_send_str("ACK_CAPTURE\r\n");

      if (pipe_suspended) {
        CMW_CAMERA_Resume(DCMIPP_PIPE1);
        pipe_suspended = 0;
      }
      cdc_send_str("CAM_RESUMED\r\n");
      tx_thread_sleep(CAPTURE_SETTLE_TICKS);

      CMW_CAMERA_Suspend(DCMIPP_PIPE1);
      pipe_suspended = 1;
      tx_thread_sleep(2);
      cdc_send_str("CAM_FROZEN\r\n");
      ...
      if (status != UX_SUCCESS) { cdc_send_str("HDR_FAIL\r\n"); continue; }
      ...
      while (remaining > 0)
      {
        if ((tx_time_get() - t_start) > 1000) break;  /* 10s timeout */
      ...
      char done[48];
      snprintf(done, sizeof(done), "XFER_%s:%d/%lu\r\n",
               remaining == 0 ? "OK" : "FAIL",
               chunks, (unsigned long)payload_size);
      cdc_send_str(done);
      dbg(done);
    }
  }
