...
"%CUBE_PROGRAMMER_PATH%\STM32_SigningTool_CLI.exe" -bin %APPLI_BIN% -nk -of 0x80000000 -t fsbl -o %APPLI_SIGNED% -align -hv 2.3 -dump %APPLI_SIGNED%
...
if "%PROGRAMMER%"=="STLINK" (
    ...
    "%CUBE_PROGRAMMER_PATH%\STM32_Programmer_CLI.exe" -c port=SWD -el "%EXT_LOADER_PATH%" -d "%FSBL_DIR%\%FSBL_SIGNED%" %FSBL_FLASH_ADDR% -d "%APPLI_DIR%\%APPLI_SIGNED%" %APPLI_FLASH_ADDR% -rst
    if %ERRORLEVEL% neq 0 (
