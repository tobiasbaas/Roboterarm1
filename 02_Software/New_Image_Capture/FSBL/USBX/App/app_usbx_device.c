...
UINT MX_USBX_Device_Stack_Init(void)
{
  ...
  /* NOTE: ux_dcd_stm32_initialize is called later in USBX_APP_Device_Init()
     AFTER MX_USB1_OTG_HS_PCD_Init() has configured the hardware.
     Calling it here before PCD init caused the DCD to link to an
     uninitialised PCD handle — removed to match the STM VENC_USB example. */
...
extern void MX_USB1_OTG_HS_PCD_Init(void);
  ...
  /* ---------- USB OTG FIFO configuration (total RAM = 1024 words) ----------
   *
   * CDC ACM endpoints (from ux_device_descriptors.h):
   *   EP1 IN (0x81) = interrupt CMD   → uses TxFIFO 1
   *   EP2 IN (0x82) = bulk DATA       → uses TxFIFO 2
   *   EP3 OUT(0x03) = bulk DATA OUT   → uses RxFIFO (shared)
   *
   *   RxFIFO    : 0x200  = 512 words = 2048 B
   *   TxFIFO 0  : 0x10   =  16 words =   64 B  (EP0 control)
   *   TxFIFO 1  : 0x02   =   2 words =    8 B  (EP1 interrupt CMD, 8B MPS)
   *   TxFIFO 2  : 0x100  = 256 words = 1024 B  (EP2 bulk DATA, 512B MPS)
   *   -----------------------------------------------
   *   Total       0x312 = 786 words  <  1024 limit           ✓
   */
  ...
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS1, 2, 0x100);
