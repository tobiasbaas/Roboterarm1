...
# Legacy-Schema (6 KPs): [base, j1, j2, j3, j4, tcp]
# Aktuelles Schema (9 KPs): [base, base_z1, base_z2, j1, j1_z1, j2, j3, j4, tcp]
LEGACY_6KP_TO_9KP_INDEX = {
    0: KP_IDX["base"],
    1: KP_IDX["j1"],
    2: KP_IDX["j2"],
    3: KP_IDX["j3"],
...
# Real-Pose Pseudo-Labeling: fehlende Keypoints NICHT heuristisch vorbesetzen.
# Nur Keypoints die vom Modell erkannt werden. Rest wird manuell annotiert.
...
def _remap_pose_keypoints_to_target_schema(keypoints, model_keypoint_count, target_keypoint_count):
    """Bringt erkannte Keypoints in das erwartete Zielschema.

    Unterstützt explizit das Legacy-6er-Schema eines älteren Pose-Modells,
    damit beim Schreiben von 9er-Labels keine Klassenverschiebung entsteht.
    """
    if model_keypoint_count == target_keypoint_count:
        return dict(keypoints), None

    # Legacy: Modell liefert 6 KPs, Ziel ist 9 KPs.
    if model_keypoint_count == 6 and target_keypoint_count == 9:
        remapped = {idx: (0.0, 0.0, 0) for idx in range(target_keypoint_count)}
        for src_idx, dst_idx in LEGACY_6KP_TO_9KP_INDEX.items():
            if src_idx in keypoints:
                remapped[dst_idx] = keypoints[src_idx]
        return remapped, "legacy_6_to_9"

    raise ValueError(
        "Nicht unterstütztes Keypoint-Schema: "
        f"Modell liefert {model_keypoint_count}, erwartet werden {target_keypoint_count}."
    )
...
def blender_masken_zu_labels():
    ...
    DEBUG: Speichert auch Zwischen-Bilder für Thesis-Dokumentation:
      - Original-Maskenbild (BGR)
      - Binäre Masken pro Klasse nach HSV-Schwellwertbildung
    """
    masks_dir = BLENDER_DATASET_DIR / "masks"
    labels_dir = BLENDER_DATASET_DIR / "labels"
    labels_dir.mkdir(parents=True, exist_ok=True)

    # DEBUG: Erstelle Verzeichnis für Thesis-Visualisierung
    debug_dir = labels_dir.parent / "debug_segmentation"
    ...
    for mask_path in mask_files:
        ...
        # DEBUG: Erstelle Unterordner für diese Maske
        mask_stem = mask_path.stem  # z.B. "mask_0001"
        mask_debug_dir = debug_dir / mask_stem
        mask_debug_dir.mkdir(parents=True, exist_ok=True)

        # DEBUG: Speichere Original-Bild
        original_path = mask_debug_dir / "00_original_bgr.png"
        ...
        with open(txt_path, "w", encoding="utf-8") as f:
            for class_id in [0, 1, 2, 3, 4]:
                ...
                # DEBUG: Speichere binäre Maske pro Klasse
                binary_path = mask_debug_dir / f"01_binary_class_{class_id}.png"
    ...
    print(f"DEBUG: Zwischen-Bilder gespeichert in: {debug_dir}")
...
def blender_real_annutieren():
    """Generiert YOLO-Pose-Labels aus Pose-Modell-Inferenz auf echten Bildern.

    Das trainierte Pose-Modell (von Blender-Daten) wird auf echten Bildern
    inferiert und liefert direkt die präzisen Keypoint-Positionen.
    So entfällt manuelle Nacharbeit in Label Studio.

    Voraussetzung: Trainiertes Pose-Modell (Blender oder Real).
    """
    from ultralytics import YOLO
    import pandas as pd

    # Pose-Modell laden (Priorität: Blender > Real)
    pose_real_path = MODEL_OUTPUT_DIR / "roboterarm_pose_real" / "weights" / "best.pt"
    pose_blender_path = MODEL_OUTPUT_DIR / "roboterarm_pose" / "weights" / "best.pt"

    if pose_blender_path.exists():
        model_path = pose_blender_path
        print(f"Nutze trainiertes Pose-Modell (Blender): {model_path}")

    elif pose_real_path.exists():
        model_path = pose_real_path
        print(f"Nutze trainiertes Pose-Modell (Real): {model_path}")
    else:
        print(f"FEHLER: Kein trainiertes Pose-Modell gefunden!")
        print(f"  Erwartet: {pose_real_path}")
        print(f"  oder:     {pose_blender_path}")
    ...
    print(f"Pose-Modell: {model_path}")
    model = YOLO(str(model_path))
    ...
    # Ground Truth CSV laden (optional, für Protokoll)
    gt_data = {}
    if GROUND_TRUTH_CSV.exists():
        df = pd.read_csv(GROUND_TRUTH_CSV)
        for _, row in df.iterrows():
    ...
    # Keypoint-Anzahl aus Blender-Pose-Labels ermitteln
    ...
    print(f"Ziel-Keypoint-Anzahl: {target_keypoint_count}")

    # Inferenz: Keypoints direkt vom Pose-Modell auslesen
    valid_pairs = []
    skipped_no_kps = 0
    skipped_low_visibility = 0
    skipped_schema_mismatch = 0
    printed_remap_notice = False

    for img_path in images:
        result = model(str(img_path), verbose=False)[0]

        # Keypoints direkt vom Pose-Modell
        if result.keypoints is None or len(result.keypoints) == 0:
            skipped_no_kps += 1
            continue
        ...
        model_keypoint_count = len(kps)
        try:
            keypoints, remap_mode = _remap_pose_keypoints_to_target_schema(
                keypoints=keypoints,
                model_keypoint_count=model_keypoint_count,
                target_keypoint_count=target_keypoint_count,
            )
        except ValueError as exc:
            skipped_schema_mismatch += 1
            if skipped_schema_mismatch == 1:
                print(f"WARNUNG: {exc}")
            continue

        if remap_mode == "legacy_6_to_9" and not printed_remap_notice:
            printed_remap_notice = True
            print(
                "Hinweis: Legacy-6KP-Modell erkannt. "
                "Keypoints werden auf das 9er-Schema remappt "
                "(Base_Z1, Base_Z2, Joint_1_Z1 bleiben ggf. 0/unsichtbar)."
            )

        # Sanity-Check: Genug sichtbare Keypoints?
        if len(visible_points) < 3:
            skipped_low_visibility += 1
            continue

        # Optional: Fehlende Keypoints heuristisch vorbesetzen
    ...
    print(f"\n{len(valid_pairs)} Bilder mit verwertbaren Keypoints")
    if skipped_no_kps:
        print(f"{skipped_no_kps} Bilder übersprungen (keine Keypoints erkannt)")
    if skipped_low_visibility:
        print(f"{skipped_low_visibility} Bilder übersprungen (zu wenige sichtbare Keypoints)")
    if skipped_schema_mismatch:
        print(f"{skipped_schema_mismatch} Bilder übersprungen (nicht unterstütztes Keypoint-Schema)")
    ...
    for split, idxs in splits.items():
        for idx in idxs:
            ...
            # Sichtbare Keypoints für BBox-Berechnung
            visible_kps = []
            ...
            # YOLO-Pose-Label schreiben: 0 cx cy w h (x y v)*N
            kp_parts = []
            for j in range(target_keypoint_count):
                if j in keypoints:
                    kx, ky, v = keypoints[j]
    ...
    print(f"\n✅ Auto-Labels aus trainiertem Pose-Modell generiert!")
    print(f"→ Training starten mit: python EdgeAI.py 2")
