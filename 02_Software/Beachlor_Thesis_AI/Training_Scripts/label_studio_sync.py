...
def _detect_json_file(export_dir: Path) -> Path:
    """Erkennt automatisch die Label-Studio JSON-Export-Datei."""
    if not export_dir.exists():
        raise FileNotFoundError(f"Export folder not found: {export_dir}")

    # Suche nach project-*-at-*.json Pattern
    json_files = list(export_dir.glob("project-*-at-*.json"))
    if json_files:
        return json_files[0]

    # Fallback: beliebige JSON
    json_files = list(export_dir.glob("*.json"))
    if json_files:
        return json_files[0]

    raise FileNotFoundError(f"No Label Studio JSON export found in {export_dir}")


def _get_keypoint_name_to_id(data: list[dict]) -> dict[str, int]:
    """Extract unique keypoint labels from JSON and assign IDs."""
    keypoint_names = set()
    for task in data:
        for anno in task.get('annotations', []):
            for result in anno.get('result', []):
                if result['type'] == 'keypointlabels':
                    for kp_label in result['value'].get('keypointlabels', []):
                        keypoint_names.add(kp_label)
    return {name: idx for idx, name in enumerate(sorted(keypoint_names))}


def _extract_image_stem_from_task(task: dict) -> Optional[str]:
    """Extract image filename stem from task data (URL or local path)."""
    data = task.get('data', {})
    image_url = data.get('image', '')
    if not image_url:
        return None
    path_part = image_url.split('/')[-1]
    stem = Path(path_part).stem
    return stem if stem else None


def _convert_json_to_yolo_labels(
    json_path: Path,
    keypoint_map: dict[str, int],
) -> Tuple[Dict[str, List[str]], int]:
    """
    Convert Label Studio JSON to YOLO Keypoint format.
    Returns: {image_stem -> [yolo_lines], num_keypoints}
    """
    ...
    for task in data:
        image_stem = _extract_image_stem_from_task(task)
        if not image_stem:
            continue

        for anno_idx, anno in enumerate(task.get('annotations', [])):
            ...
            for result in results:
                if result['type'] == 'rectanglelabels':
                    bboxes[result['id']] = result
                elif result['type'] == 'keypointlabels':
                    parent_id = result.get('parentID')
                    if parent_id:
                        keypoints[parent_id].append(result)

            yolo_lines = []

            for bbox_id, bbox_result in bboxes.items():
                bbox_class_id = 0  # Single class: robot_arm

                # Label Studio liefert (x, y) als obere linke Ecke;
                # YOLO erwartet (x_center, y_center).
                x_left = bbox_result['value'].get('x', 0) / 100.0
                y_top = bbox_result['value'].get('y', 0) / 100.0
                width = bbox_result['value'].get('width', 1) / 100.0
                height = bbox_result['value'].get('height', 1) / 100.0
                ...
                kps = keypoints.get(bbox_id, [])
                kp_str_parts = [f"{bbox_class_id} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}"]

                for kp_name in sorted(keypoint_map.keys()):
                    kp_found = None
                    for kp in kps:
                        if kp_name in kp['value'].get('keypointlabels', []):
                            kp_found = kp
                            break

                    if kp_found:
                        kp_x = kp_found['value']['x'] / 100.0
                        kp_y = kp_found['value']['y'] / 100.0
                        kp_vis = 2  # 2 = sichtbar (YOLO-Pose Konvention)
                    else:
                        kp_x = 0.0
                        kp_y = 0.0
                        kp_vis = 0  # 0 = nicht annotiert

                    kp_str_parts.append(f"{kp_x:.6f} {kp_y:.6f} {kp_vis}")

                yolo_lines.append(" ".join(kp_str_parts))

            if yolo_lines:
                if anno_idx > 0:
                    key = f"{image_stem}__anno_{anno_idx}"
                else:
                    key = image_stem
                labels_by_stem[key] = yolo_lines

    return labels_by_stem, num_keypoints
...
def import_pose_yolo(
    ...
    """Importiert Pose-Labels direkt aus Label-Studio JSON in train/val inkl. Normalisierung."""
    json_path = _detect_json_file(export_dir)
    ...
    # 1) Convert JSON to YOLO labels
    keypoint_map = _get_keypoint_name_to_id(_read_json(json_path))
    labels_by_stem, num_keypoints = _convert_json_to_yolo_labels(json_path, keypoint_map)

    # 2) Import converted labels
    for label_key, yolo_lines in labels_by_stem.items():
        stem = label_key.split("__")[0] if "__" in label_key else label_key
    ...
    # 2b) Stale labels entfernen, damit nur aktueller Export im Training bleibt.
    for p in train_lbl_dir.glob("*.txt"):
