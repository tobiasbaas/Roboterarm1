...
## Pfadsystem (aktualisiert)

Die Trainingsskripte sind jetzt auf ein robustes, rechnerunabhaengiges Pfadsystem umgestellt.

### 1) Zentrale Basis

- Basis ist immer der Speicherort des jeweiligen Skripts (nicht das aktuelle Terminal-Verzeichnis).
- Dadurch funktionieren die Skripte auch dann stabil, wenn sie aus verschiedenen Ordnern gestartet werden.

### 2) Model-Runs statt fester Ordnernamen

- `EdgeAI.py` fuehrt unter `Trained_Models/AI-Models/runs_index.json` mit, welcher Run aktuell der neueste ist.
- `Umrisse_in_Polygone.py` und `Validierung.ipynb` nutzen diese Information zuerst.
- Fallback bleibt erhalten, falls `runs_index.json` fehlt oder unvollstaendig ist.

Prioritaet bei der Modellwahl:
...
`label_studio_sync.py` erkennt Standard-Exportordner automatisch in:

- `~/Downloads/project-*-at-*`

Dabei werden fuer Pose verschiedene Exportstrukturen unterstuetzt (z. B. `labels/`, `obj_train_data/`, `data/labels/` oder rekursiv gefundene `*.txt`).
Fuer Segmentierung wird automatisch eine passende COCO-JSON im Exportordner gesucht.

### 5) Notebook-Pfade (Validierung)

`Validierung.ipynb` verwendet ebenfalls `runs_index.json`, um automatisch die neuesten Seg-/Pose-Modelle zu laden.
Zusatz-Fallbacks bleiben aktiv, falls ein Run (noch) nicht im Index steht.

### 6) Blender-Ausgabe-Pfade

In der Blender-Generierung werden Ausgabepfade bevorzugt relativ zur `.blend`-Datei aufgeloest.
Dadurch bleibt das Dataset auch nach Verschieben des Projekts konsistent.
...
#### Rueckimport nach Label Studio (automatisiert)

Fuer den Rueckweg von Label Studio ins Trainingsdataset gibt es jetzt ein
dediziertes Skript:

`Training_Scripts/label_studio_sync.py`

Damit wird der manuelle Copy/Paste-Schritt ersetzt. Das Skript:

- importiert **Pose** aus Label-Studio-**YOLO-Export**
- importiert **Segmentierung** aus Label-Studio-**COCO-Export**
- mappt Dateinamen automatisch (z. B. `<hash>__pose_0123.txt` -> `pose_0123.txt`)
- erstellt vor jeder Aenderung ein Backup
- normalisiert Pose-Labels auf die erwartete Spaltenzahl aus `real_pose_dataset.yaml`

##### Empfohlene Formate in Label Studio

- **Pose-Projekt:** `YOLO` exportieren
- **Segmentierungs-Projekt:** `COCO` exportieren
- Export **ohne Bilder** ist in Ordnung, solange JSON/TXT-Annotationen gefuellt sind.

##### Schnellstart (beides in einem Lauf)

```powershell
cd .\Training_Scripts   # aus Beachlor_Thesis_AI starten

python .\label_studio_sync.py all \
   --pose-export-dir "<Pfad_zum_pose_yolo_export_ordner>" \
   --seg-coco "<Pfad_zum_seg_coco_export_ordner_oder_json>"
```

##### Empfohlen: interaktiver Start mit automatischen Defaults
...
Standardwerte fuer Exportpfade werden automatisch aus dem neuesten Label-Studio-Download vorgeschlagen.

##### Nur Pose (YOLO)

```powershell
python .\label_studio_sync.py pose-yolo \
   --export-dir "<Pfad_zum_pose_yolo_export_ordner>"
```

##### Nur Segmentierung (COCO)

```powershell
python .\label_studio_sync.py seg-coco \
   --coco "<Pfad_zum_seg_coco_export_ordner_oder_json>"
```

##### Trockenlauf (keine Dateien schreiben)

```powershell
python .\label_studio_sync.py --dry-run all \
   --pose-export-dir "<Pfad_zum_pose_yolo_export_ordner>" \
   --seg-coco "<Pfad_zum_seg_coco_export_ordner_oder_json>"
```

##### Danach Finetuning starten
...
#### Neuer interaktiver Menue-Workflow (empfohlen)

Der Konverter hat jetzt ein Start-Menue. Einfach ohne Argumente starten:
...
Dann im Menue waehlen:

1. Nur Segmente umwandeln
2. Nur Pose umwandeln (train + val)
3. Segmente + Pose umwandeln (beides)

Das Skript setzt danach automatisch die passenden Standardpfade:

- Segmente: `real_dataset/images/all` + `real_dataset/labels/all`
- Pose: `real_pose_dataset/images/train|val` + `real_pose_dataset/labels/train|val`
- Ausgabe: `label_studio_import/`

Zusatzabfragen im Menue:

- Root-Pfad (Default: `Training_Scripts`)
- Image-Host (Default: `http://127.0.0.1:3000`)
- Import-Modus `predictions` oder `annotations`
- Bei Segmenten: `polygon` oder `brush`
...
#### CLI-Workflow (optional, ohne Menue)
...
Fuer portable Setups auf verschiedenen Rechnern kannst du bei Bedarf diese
...
Die Blender-Daten enthalten `joint_data.csv` mit Gelenkwinkeln (angle_j1-j4). Die Pose-Labels nutzen aktuell ein 9-Keypoint-Schema (inkl. `Base_Z1`, `Base_Z2`, `Joint_1_Z1`) und liegen vorgeneriert im Ordner `dataset/labels_pose/` im YOLO-Format.
