...
void HAL_PCD_MspInit(PCD_HandleTypeDef* hpcd)
{
  if(hpcd->Instance==USB1_OTG_HS)
  {
    ...
    /* ---- Matching official ST Ux_Device_Video example (DK board) ---- */

    /* Enable VDDUSB with voltage monitor */
  ...
  */
    ...
    /* USB OTG HS AHB clock from HSE direct (48 MHz) */
    ...
    /* USB PHY1 reference clock from HSE direct */
    ...
    /* Full USB + PHY wrapper reset sequence (matching official ST examples) */
    LL_AHB5_GRP1_ForceReset(0x00800000);    /* PHY wrapper reset */
    __HAL_RCC_USB1_OTG_HS_FORCE_RESET();
    __HAL_RCC_USB1_OTG_HS_PHY_FORCE_RESET();

    LL_RCC_HSE_SelectHSEDiv2AsDiv2Clock();   /* Select HSE/2 for div2 clock */
    LL_AHB5_GRP1_ReleaseReset(0x00800000);   /* Release PHY wrapper reset */
    ...
    /* PHY reference clock = 24 MHz + common block clk + PHY enable */
    USB1_HS_PHYC->USBPHYC_CR &= ~(0x7 << 0x4);
    USB1_HS_PHYC->USBPHYC_CR |= (0x1 << 16) |   /* COMMONCKEN */
                                 (0x2 << 4)  |   /* FSEL = 24 MHz */
                                 (0x1 << 2)  |   /* bit 2 */
                                  0x1U;           /* bit 0 */
    ...
    /* Required few clock cycles before releasing USB reset */
    HAL_Delay(1);
    ...
    /* USB1_OTG_HS interrupt Init — priority 7 (matching official ST examples) */
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef* hltdc)
{
  ...
  {
    /* LTDC pixel clock: PLL1(1200MHz) / 48 = 25 MHz */
    ...
    HAL_NVIC_SetPriority(LTDC_UP_ERR_IRQn, 0, 0);
    ...
    /* LCD_ONOFF (PQ3) and LCD_BL_CTRL (PQ6) */
    GPIO_InitStruct.Pin = GPIO_PIN_3 | GPIO_PIN_6;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOQ, &GPIO_InitStruct);

    /* Override PG13 (LCD_DE) from AF14 to GPIO output PP */
    GPIO_InitStruct.Pin = GPIO_PIN_13;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOG, &GPIO_InitStruct);

    /* Drive LCD control signals HIGH */
    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_1, GPIO_PIN_SET);  /* LCD_NRST */
    HAL_GPIO_WritePin(GPIOQ, GPIO_PIN_3, GPIO_PIN_SET);  /* LCD_ONOFF */
    HAL_GPIO_WritePin(GPIOG, GPIO_PIN_13, GPIO_PIN_SET); /* LCD_DE */
    HAL_GPIO_WritePin(GPIOQ, GPIO_PIN_6, GPIO_PIN_SET);  /* LCD_BL_CTRL */
  }
}
